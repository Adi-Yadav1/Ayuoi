...
export default function DietAI() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>(MOCK_MESSAGES);
  const [isSending, setIsSending] = useState(false);
  // Controls whether the hero card is visible (hides once user starts chatting)
  ...
  // Collapse the hero card the first time the user sends a message
  const collapseHero = () => {
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      {/* ── Hero card — fades out after first message ── */}
      {heroVisible && (
        <Animated.View
          style={[styles.heroCard, { opacity: Animated.multiply(fadeAnim, heroOpacity) }]}
        >
          <Text style={styles.heroTitle}>Today&apos;s Focus</Text>
          <Text style={styles.heroBody}>
            Keep meals warm, lightly spiced, and easy to digest. Prioritize cooked
      ...
      {/* ── Input bar ── */}
      <Animated.View style={[styles.inputBar, { bottom: keyboardHeight }]}>
        <View style={[styles.inputWrap, isSending && styles.inputWrapDisabled]}>
          <TextInput
...
// ── Extracted message bubble with fade-in animation ─────────────────────────
function MessageBubble({ item }: { item: ChatMessage }) {
  ...
  return (
    ...
    >
      <View style={isAssistant ? styles.avatarAssistant : styles.avatarUser}>
        <Text style={isAssistant ? styles.avatarTextAssistant : styles.avatarTextUser}>
          {isAssistant ? "A" : "U"}
        </Text>
      </View>
      <View style={isAssistant ? styles.bubbleAssistant : styles.bubbleUser}>
        <Text style={isAssistant ? styles.bubbleTextAssistant : styles.bubbleTextUser}>
          {item.text}
        </Text>
        <Text style={isAssistant ? styles.timeAssistant : styles.timeUser}>
          {item.time}
        </Text>
      </View>
    </Animated.View>
